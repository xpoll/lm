...
/**
 * Created by IntelliJ IDEA. User: AnsonChan Date: 14-4-29
 */
@Slf4j
public class AgentImpl implements Agent, InitializingBean, ApplicationContextAware {
	...
	@Override
	public LinkedHashMap<String, Agent.ParamInfo> getParamsInfo(String key) {
		...
		LinkedHashMap<String, Agent.ParamInfo> paramsInfo = Maps.newLinkedHashMap();
		for (String paramKey : methodInfo.getParams().keySet()) {
			...
			paramsInfo.put(paramKey,
					new Agent.ParamInfo(richParamInfo.isOptional(), richParamInfo.getClazz().getName()));
		}
		return paramsInfo;
	}
