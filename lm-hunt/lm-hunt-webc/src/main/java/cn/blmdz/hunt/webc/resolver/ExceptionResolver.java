...
public class ExceptionResolver extends ExceptionHandlerExceptionResolver {
	...
	private Error buildError(Exception exception) {
		if (exception instanceof JsonResponseException) {
			...
			return new Error(status, Objects.firstNonNull(this.messageSources.get(jsonEx.getMessage()), ERROR_DEFAULT));
		}
		if (exception instanceof ServiceException) {
			return new Error(Integer.valueOf(500), Objects.firstNonNull(this.messageSources.get(exception.getMessage()), ERROR_DEFAULT));
		}
		if (exception instanceof NotFound404Exception) {
			return new Error(Integer.valueOf(404), Objects.firstNonNull(this.messageSources.get(exception.getMessage()), ERROR_PAGE_NOT_FOUND));
		}
		if (exception instanceof UnAuthorize401Exception) {
			return new Error(Integer.valueOf(401), Objects.firstNonNull(this.messageSources.get(exception.getMessage()), ERROR_USER_NOT_AUTH));
		}
		if (exception instanceof BindException) {
			BindException bindException = (BindException) exception;
			BindingResult result = bindException.getBindingResult();
			return new Error(Integer.valueOf(400), Objects.firstNonNull(this.messageSources.get(result.getFieldError().getDefaultMessage()), ERROR_DEFAULT));
		}
		if (exception instanceof UserNotLoginException) {
			return new Error(Integer.valueOf(500), ERROR_USER_NOT_LOGIN);
		}
		if (exception instanceof IllegalArgumentException) {
			return new Error(Integer.valueOf(400), Objects.firstNonNull(this.messageSources.get(exception.getMessage()), ERROR_ARGS));
		}
