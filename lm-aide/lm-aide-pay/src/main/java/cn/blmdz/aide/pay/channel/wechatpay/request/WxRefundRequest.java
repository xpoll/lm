...
public class WxRefundRequest extends WxRequest {
   ...
   public Response refund() {
      Response<WxPayRefundResponse> result = new Response();
      super.nonceStr();
      super.sign();
      ...
      try {
         String body = HttpsUtil.build(this.caFilePath, this.certFilePath).callHttps(preXml, this.config.getMchId(), this.config.getRefundGateway()).getResContent();
