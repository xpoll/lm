package io.terminus.lib.pay.channel.wechatpay.manager;
...
import io.terminus.lib.pay.channel.wechatpay.OpenIdUtil;
...
import io.terminus.lib.pay.channel.wechatpay.request.WxJsApiRequest;
import io.terminus.lib.pay.channel.wechatpay.request.WxPrePayRequest;
import io.terminus.lib.pay.channel.wechatpay.request.WxRefundRequest;
...
public class WechatpayManager {
   ...
   public RedirectInfo prePay(String channel, String subject, String content, String tradeNo, Integer fee, String notifyUrl, Long buyerId, WxToken wxToken) {
      ...
      try {
         ...
         if(channel.endsWith("jsapi")) {
            String xml = WxPrePayRequest.build(wxToken).notifyUrl(notifyUrl).body(subject.substring(0, Math.min(50, subject.length()))).outTradeNo(tradeNo).totalFee(fee).ip("127.0.0.1").tradeType(WxPrePayRequest.TradeType.JSAPI).openId(OpenIdUtil.getOpenId(buyerId)).prePayParams(wxToken.getGateway());
            info.setResult(xml);
         } else {
            String xml = WxPrePayRequest.build(wxToken).notifyUrl(notifyUrl).body(subject.substring(0, Math.min(50, subject.length()))).outTradeNo(tradeNo).totalFee(fee).productId("1").timeStart(DateTime.now().toDate()).timeExpire(DateTime.now().plusDays(1).toDate()).attach("attach").tradeType(WxPrePayRequest.TradeType.NATIVE).prePayParams(wxToken.getGateway());
            info.setResult(xml);
         }
   ...
   public RedirectInfo mockPay(String channel, String subject, String content, String tradeNo, Integer fee, String notifyUrl, Long buyerId, WxToken wxToken) {
      RedirectInfo info = new RedirectInfo();

      try {
         OpenIdUtil.putOpenId(buyerId, "osvRcuFMHhG-31pJuIVuyi6N0VbE");
         WxPrePayRequest request = WxPrePayRequest.build(wxToken).notifyUrl(notifyUrl).body(subject.substring(0, Math.min(50, subject.length()))).outTradeNo(tradeNo).totalFee(fee).ip("127.0.0.1").tradeType(WxPrePayRequest.TradeType.JSAPI).openId(OpenIdUtil.getOpenId(buyerId));
   ...
   public RedirectInfo pay(String channel, String tradeNo, WxPrePayResponse prePayRes, WxToken wxToken) {
      return channel.endsWith("jsapi")?this.handleJsapi(channel, prePayRes, wxToken):this.handleNative(channel, tradeNo, prePayRes, wxToken);
   }

   public RedirectInfo refund(Long refundTrackId, String caFilePath, String certFilePath, String channel, String tradeNo, String refundNo, String paymentCode, Integer refundAmount, Integer totalFee, WxToken wxToken) {
      RedirectInfo info = new RedirectInfo();
      ...
      try {
         ...
         String xml = WxRefundRequest.build(wxToken).caFilePath(caFilePath).certFilePath(certFilePath).outTradeNo(tradeNo).transactionId(paymentCode).outRefundNo(refundNo).refundFee(refundAmount).totalFee(totalFee).refundParam();
         info.setResult(xml);
   ...
   public RedirectInfo mockRefund(Long refundTrackId, String channel, String tradeNo, String refundNo, String paymentCode, Integer refundAmount, Integer totalFee, WxToken wxToken) {
      RedirectInfo info = new RedirectInfo();

      try {
         this.validRefundParam(refundTrackId, channel, tradeNo, refundNo, paymentCode, refundAmount);
         this.checkParam(wxToken, (String)null, Integer.valueOf(TradeType.REFUND.value()));
         String xml = WxRefundRequest.build(wxToken).outTradeNo(tradeNo).transactionId(paymentCode).outRefundNo(refundNo).refundFee(refundAmount).totalFee(totalFee).mockUrl(wxToken.getRefundGateway());
         info.setResult(xml);
         info.setIsRedirectNow(Boolean.TRUE);
         info.setChannel(channel);
      } catch (PayRequestParamException var11) {
   ...
   private void validRefundParam(Long refundTrackId, String channel, String tradeNo, String refundNo, String paymentCode, Integer refundAmount) {
   ...
   private RedirectInfo handleJsapi(String channel, WxPrePayResponse prePayRes, WxToken wxToken) {
      ...
      try {
         ...
         log.error("handle js api fail where channel: {} WxPrePayResponse:{} ,error:{}", new Object[]{channel, prePayRes, var6.getMessage()});
         info.setError(var6.getMessage());
      } catch (Exception var7) {
         log.error("handle js api fail where channel: {} WxPrePayResponse:{} ,cause:{}", new Object[]{channel, prePayRes, Throwables.getStackTraceAsString(var7)});
   ...
   private RedirectInfo handleNative(String channel, String tradeNo, WxPrePayResponse prePayRes, WxToken wxToken) {
      ...
      try {
         ...
         info.setIsRedirectNow(Boolean.FALSE);
         info.setResult(JsonMapper.nonEmptyMapper().toJson(MapBuilder.of().put("qrcodeUrl", codeUrl, "tradeNo", tradeNo).map()));
         info.setChannel(channel);
      } catch (Exception var7) {
         log.error("handle native where channel:{} tradeNo: {} WxPrePayResponse: {} fail,cause:{}", new Object[]{channel, tradeNo, prePayRes, Throwables.getStackTraceAsString(var7)});
