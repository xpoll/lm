...
import io.terminus.lib.pay.channel.wechatpay.dto.WxPayRefundDto;
import io.terminus.lib.pay.channel.wechatpay.dto.WxPayRefundQueryResponse;
...
public class WxRefundQueryRequest extends WxRequest {
   ...
   public Response query() {
      Response<WxPayRefundQueryResponse> result = new Response();
      super.nonceStr();
      super.sign();
      ...
      String body = HttpRequest.post(this.config.getQueryRefundGateway()).send(preXml).trustAllCerts().trustAllHosts().body();
      log.debug("wx refund query body: {}", body);
      if(verify(body, this.config)) {
         WxPayRefundQueryResponse refundQueryResponse = (WxPayRefundQueryResponse)parse(body, WxPayRefundQueryResponse.class);
         if(refundQueryResponse.isSuccess()) {
            refundQueryResponse.setWxPayRefundDtos(this.paresFromMap(fromXML(body), Integer.valueOf(refundQueryResponse.getRefundCount())));
         }
   ...
   private List paresFromMap(Map map, Integer num) {
