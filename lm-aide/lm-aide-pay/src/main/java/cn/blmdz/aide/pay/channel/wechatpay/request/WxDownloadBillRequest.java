...
public class WxDownloadBillRequest extends WxRequest {
	...
	public Response<WxPayBillResponse> query() {
		Response<WxPayBillResponse> result = new Response<WxPayBillResponse>();
		super.nonceStr();
		super.sign();
		String preXml = toXml(this.params);
		log.debug("wx download bill url: {}", DOWLOAD_BILL_URL);
		log.debug("wx download bill xml: {}", preXml);
		String body = HttpRequest.post(DOWLOAD_BILL_URL).send(preXml).trustAllCerts().trustAllHosts().body();
		log.debug("wx refund query body: {}", body);
		if (body.indexOf("xml") != -1) {
			@SuppressWarnings("unchecked")
			Map<String, Object> error = (Map<String, Object>) fromXML(body);
	...
	public static enum BillType {
		ALL("ALL"), SUCCESS("SUCCESS"), REFUND("REFUND");
