  ...
  implements ItemDumpService
{
  ...
  public Response<Boolean> fullDump()
  {
    try
    {
      ...
      log.error("failed to execute items full dump, cause: {}", Throwables.getStackTraceAsString(e));
    }
  ...
  private int doIndex(String since)
  {
    ...
    while (true) {
      ...
      List itemIds = Lists.transform(items, new Function<Item, Long>()
      {
        public Long apply(Item item) {
          return item.getId();
        }
      });
      List itemAttributes = this.itemAttributeDao.findByItemIds(itemIds);
      ...
      for (Item item : items) {
        try {
          if (item.getStatus().intValue() > 0) {
            IndexedItem indexedItem = (IndexedItem) this.indexedItemFactory.create(item, byItemId.get(item.getId()), new Object[0]);
