...
import cn.blmdz.wolf.parana.cache.BrandCacher;
import cn.blmdz.wolf.parana.cache.CategoryBindingCacher;
import cn.blmdz.wolf.parana.cache.FrontCategoryCacher;
...
import cn.blmdz.wolf.parana.search.dto.AggNav;
import cn.blmdz.wolf.parana.search.dto.Chosen;
import cn.blmdz.wolf.parana.search.dto.GroupedAggNav;
import cn.blmdz.wolf.parana.search.dto.IdAndName;
import cn.blmdz.wolf.parana.search.dto.NameAndCount;
import cn.blmdz.wolf.parana.search.dto.SearchWithAggs;
import cn.blmdz.wolf.parana.search.dto.SearchedItem;
...
public class ItemSearchReadServiceImpl implements ItemSearchReadService {
	...
	public ItemSearchReadServiceImpl(SearchItemProperties searchItemProperties, BaseItemQueryBuilder itemQueryBuilder,
			...
			FrontCategoryCacher frontCategoryCacher, BrandCacher brandCacher) {
	...
	public Response searchWithAggs(Integer pageNo, Integer pageSize, String templateName, Map params) {
		...
		if (StringUtils.hasText(frontCategoryId)) {
			...
			if (CollectionUtils.isEmpty(backCategoryIds)) {
				SearchWithAggs searchWithAggs = new SearchWithAggs();
				searchWithAggs.setChosen(
						Lists.newArrayList(new Chosen[] { new Chosen(3, frontCategoryId, frontCategory.getName()) }));
				searchWithAggs.setBackCategories(Collections.emptyList());
				searchWithAggs.setEntities(Paging.empty(SearchedItem.class));
				searchWithAggs.setBrands(Collections.emptyList());
				searchWithAggs.setAttributes(Collections.emptyList());
		...
		WithAggregations<SearchedItem> withAggs = this.searcher.searchWithAggs(this.searchItemProperties.getIndexName(),
	...
	private List makeCategoryNavs(List<Bucket> catAggs, Set chosenCategories) {
				...
				if (!chosenCategories.contains(key)) {
					try {
						...
						log.error("failed to build cat navs for bucket(key={}),cause:{}", key,
								Throwables.getStackTraceAsString(var10));
					}
				}
			}
	...
	private List makeGroupedAttrNavs(List<Bucket> attrAggs, String chosenAttrs) {
		if (CollectionUtils.isEmpty(attrAggs)) {
			...
			Multimap<Object, AggNav> byKey = Multimaps.index(attrNavs, new Function<AggNav, Object>() {
			...
			for (Object key : Iterables.limit(byKey.keySet(), 5)) {
				...
				for (AggNav aggNav : Iterables.limit(aggNavs, 10)) {
					nameAndCounts
							.add(new NameAndCount(aggNav.getName(), Integer.valueOf(aggNav.getCount().intValue())));
				}
	...
	private List makeBrandNavs(List<Bucket> brandAggs) {
