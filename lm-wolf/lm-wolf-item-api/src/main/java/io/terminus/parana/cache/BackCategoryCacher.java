...
public class BackCategoryCacher {
   ...
   private final LoadingCache backCategoryCache;
   ...
   public BackCategoryCacher(final BackCategoryReadService backCategoryReadService, @Value("${cache.duration: 60}") Integer duration) {
      this.backCategoryCache = CacheBuilder.newBuilder().expireAfterWrite((long)duration.intValue(), TimeUnit.MINUTES).build(new CacheLoader() {
      ...
      this.childrenCache = CacheBuilder.newBuilder().expireAfterAccess((long)duration.intValue(), TimeUnit.MINUTES).build(new CacheLoader() {
