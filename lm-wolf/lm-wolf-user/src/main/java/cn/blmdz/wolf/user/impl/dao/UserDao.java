...
public class UserDao extends MyBatisDao<User> {
	...
	public Integer updateStatus(Long userId, Integer status) {
		return Integer.valueOf(this.getSqlSession().update(this.sqlId("updateStatus"),
				ImmutableMap.of("id", userId, "status", status)));
	}

	public Integer batchUpdateStatus(List<Long> userIds, Integer status) {
		return Integer.valueOf(this.getSqlSession().update(this.sqlId("batchUpdateStatus"),
				ImmutableMap.of("ids", userIds, "status", status)));
	}

	public Integer updateType(Long userId, int type) {
		return Integer.valueOf(this.getSqlSession().update(this.sqlId("updateType"),
				ImmutableMap.of("userId", userId, "type", Integer.valueOf(type))));
	}

	public void updateTags(Long userId, Map tags) {
		this.getSqlSession().update(this.sqlId("updateTags"),
				ImmutableMap.of("userId", userId, "tagsJson", JsonMapper.JSON_NON_DEFAULT_MAPPER.toJson(tags)));
	}

	public void updateRoles(Long userId, List roles) {
		this.getSqlSession().update(this.sqlId("updateRoles"),
				ImmutableMap.of("userId", userId, "rolesJson", JsonMapper.JSON_NON_DEFAULT_MAPPER.toJson(roles)));
	}
}
