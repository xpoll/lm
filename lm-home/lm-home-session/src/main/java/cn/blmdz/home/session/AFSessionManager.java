...
public final class AFSessionManager {
   ...
   private AFSessionManager(Configuration configuration) {
      ...
      if("redis".equals(configuration.getSource())) {
         if(!configuration.isCluster().booleanValue() && configuration.getSessionRedisHost() == null) {
            throw new IllegalStateException("You have chosen redis source, but not set [session.redis.host] at session.properties");
         } else if(!configuration.isCluster().booleanValue() && configuration.getSessionRedisPort() == null) {
            throw new IllegalStateException("You have chosen redis source, but not set [session.redis.port] at session.properties");
         } else {
   ...
   public boolean save(String prefix, String id, Map snapshot, int maxInactiveInterval) {
