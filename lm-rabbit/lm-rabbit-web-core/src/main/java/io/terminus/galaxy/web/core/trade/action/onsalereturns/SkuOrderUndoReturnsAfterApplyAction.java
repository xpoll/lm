 ...
 * 取消未处理的退货申请
 *
 * 退货单: 更新状态为SKU_RETURNS_UNDO, 更新操作集合(没有后续操作)
 * 子订单: 不更新状态, 重新打开所有操作
 ...
 */
@Component
public class SkuOrderUndoReturnsAfterApplyAction extends OnSaleReturnActionBase implements TradeAction<Boolean> {
    ...
    @Autowired
    public SkuOrderUndoReturnsAfterApplyAction(
    ...
    public Boolean execute(Map<String, Serializable> context) {
         ...
         * 退货单: 更新状态为SKU_RETURNS_UNDO, 更新操作集合(没有后续操作)
         */
        skuOrderRefund.setNodeInstanceId(OrderStatus.SKU_RETURNS_APPLY_UNDO.value());
