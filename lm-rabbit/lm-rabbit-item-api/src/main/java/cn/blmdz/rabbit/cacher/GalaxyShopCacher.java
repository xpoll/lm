...
public class GalaxyShopCacher {
	...
	@Autowired
	public GalaxyShopCacher(final ShopReadService shopReadService, final GalaxyItemReadService galaxyItemReadService,
			@Value("${cache.shop.duration-in-minutes:3}") Long duration) {
		this.shopItemCountCache = CacheBuilder.newBuilder().expireAfterWrite(duration, TimeUnit.MINUTES)
		...
		this.shopCache = CacheBuilder.newBuilder().expireAfterWrite(duration, TimeUnit.MINUTES)
